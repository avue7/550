...
// Prototypes:
void get_RedNovas_inputs(vector<vertex> &RedNova_vertices,
  vector<list<edge>> &RedNova_adj_list, int &total_cost, int n);
void get_StarTraces_inputs(vector<vertex> &StarTrace_vertices,
  vector<list<edge>> &StarTrace_adj_list, int &total_cost, int m);
largest_edge largest_weight_edge(int u, int v, vector<vertex> &vertices,
...
void get_RedNovas_inputs(vector<vertex> &RedNova_vertices,
  ...
  for (int i = 0; i < (n-1); i++){
    ...
    // Update total cost for 10 years
...
largest_edge largest_weight_edge(int u, int v, vector<vertex> &vertices,
  ...
  while(true){
    ...
    // Check the current weight, if it is bigger then set the weight to
    // the current max weight edge and get that edge's parent.
    // Then repeat until we have fully tranverse up the tree.
    if (cur_weight > cur_max_weight_edge.weight){
